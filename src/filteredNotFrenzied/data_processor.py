...
PDFs in Volume + mdpi_papers meta table
   ↓ (parse_pdfs_with_ai)
ai_parsed_docs_table (JSON)
   ↓ (process_chunks)
mdpi_chunks_table (clean text + metadata)
...
class DataProcessor:
    ...
    def process_chunks(self) -> None:
        ...
        chunks_df.write.mode("append").saveAsTable(mdpi_chunks_table)
        logger.info(f"Saved chunks to {mdpi_chunks_table}")

        # Enable Change Data Feed
        self.spark.sql(f"""
            ALTER TABLE {mdpi_chunks_table}
            SET TBLPROPERTIES (delta.enableChangeDataFeed = true)
        """)
        logger.info(f"Change Data Feed enabled for {mdpi_chunks_table}")

    def parse_and_process(self) -> None:
        """
        Complete workflow: parse PDFs and process chunks.
        """
